...
export default {
  data() {
    return {
      ...
      drawer: true,
      links: [
        { icon: "mdi-calendar-blank", text: "Current", route: "/current" },
        {
  ...
  methods: {
    async addCity() {
      ...
      await this.$store.dispatch("getLatitudeLongitude", {
        city: this.search,
      });
      await this.$store.dispatch("airPollution/currAirPollutionCurrCity", {
        lat: this.latitude,
        lon: this.longitude,
      });
      await this.$store.dispatch("airPollution/forecastAirPollutionCurrCity", {
        lat: this.latitude,
        lon: this.longitude,
      });
  ...
  computed: {
    ...
    latitude() {
      return this.$store.getters.getGeocoding.lat;
    },
    longitude() {
      return this.$store.getters.getGeocoding.lon;
    },
    currPollution() {
      return this.$store.getters["airPollution/getCurrAirPollutionCurrCity"];
    },
    forecastPollution() {
      return this.$store.getters[
        "airPollution/getForecastAirPollutionCurrCity"
      ];
    },
  },
  created() {
